        ...
        GeometryReader { geometry in
            VStack(spacing: 0) {
                ...
                if self.headerContent != nil || self.showCancelButton {
                    ...
                    .padding(.horizontal)
                    .padding(.top, self.resizeable ? 8 : 20)
                    .padding(.bottom, self.isBottomPosition() ? geometry.safeAreaInsets.bottom : 0)
                }
                
                self.mainContent
                    .transition(.move(edge: .bottom))
                    .animation(Animation.spring(response: 0.25, dampingFraction: 0.75, blendDuration: 1))
